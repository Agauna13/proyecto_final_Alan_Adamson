...
@section('content')
    <section
        ...
        <div class="max-w-3xl mx-auto mb-10 text-center animate-fade-in-down">
            ...
            <p class="text-gray-300 text-sm">Confirma tu pedido. Puedes añadir extras o entrantes opcionales antes de
                finalizar.</p>
        </div>

        @if ($reservaData || $mesa)
            <div
                class="max-w-3xl mx-auto mb-6 text-sm text-gray-300 bg-gray-800/40 p-4 rounded shadow border border-yellow-400/20">
                @if ($reservaData)
                    <p><strong>Reserva:</strong> {{ $reservaData['nombre'] }} — {{ $reservaData['fecha'] }} a las
                        {{ $reservaData['hora'] }}</p>
                @endif
                @if ($mesa)
        ...
        <form action="{{ route('pedidos.store') }}" method="POST" class="max-w-3xl mx-auto space-y-8 animate-fade-in pb-28"
            id="pedidoForm">
            @csrf
            ...
            <div>
                ...
                @foreach ($productosUnicos as $index => $producto)
                    <div
                        class="bg-gray-800/60 border border-yellow-500/30 rounded-xl p-4 mb-6 shadow hover:shadow-yellow-400/20 transition">
                        <input type="hidden" name="productos[{{ $index }}][producto_id]"
                            value="{{ $producto->id }}">
                        <input type="hidden" name="productos[{{ $index }}][precio_unitario]"
                            value="{{ $producto->precio }}" class="precio-unitario" data-precio="{{ $producto->precio }}">
                        ...
                        @if ($producto->extras->isNotEmpty())
                            <div class="mt-4">
                                ...
                                @if ($producto->categoria === 'Licores')
                                    {{-- Si es de la categoría licores, usar un <select> estilizado para elegir solo uno --}}
                                    <div class="relative mt-2">
                                        <select name="productos[{{ $index }}][extra_licor]"
                                            class="block w-full rounded-md border border-yellow-400 bg-gray-700 text-white shadow-sm focus:border-yellow-500 focus:ring focus:ring-yellow-500/50">
                                ...
                                @else
                                    {{-- Para otras categorías, mantener contadores --}}
                                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
                                        @foreach ($producto->extras as $extra)
                                            <div
                                                class="flex items-center justify-between bg-gray-700/60 rounded px-3 py-2 border border-yellow-500/10">
                                                <label class="text-sm text-gray-200">{{ $extra->nombre }}
                                                    (+{{ number_format($extra->precio, 2) }} €)</label>
                                                <div class="flex items-center gap-2">
                                                    <button type="button"
                                                        class="decrease px-3 py-1 bg-yellow-500 text-black font-bold rounded-l hover:bg-yellow-400">−</button>
                                                    <input type="number"
                                                        name="productos[{{ $index }}][extras_por_unidad][{{ $extra->id }}]"
                                                        min="0" value="0" data-precio="{{ $extra->precio }}"
                                                        class="cantidad-input w-14 sm:w-16 text-center text-black font-semibold rounded-none">
                                                    <button type="button"
                                                        class="increase px-3 py-1 bg-yellow-500 text-black font-bold rounded-r hover:bg-yellow-400">+</button>
                                                </div>
                                            </div>
                                        @endforeach
                                    </div>
                ...
                <div>
                    ...
                    <div class="space-y-4">
                        @foreach ($entrantes as $entrante)
                            <div
                                class="flex items-center justify-between bg-gray-800/40 border border-yellow-500/10 rounded px-4 py-2">
                                <label class="text-sm text-gray-200">{{ $entrante->nombre }}
                                    ({{ number_format($entrante->precio, 2) }} €)</label>
                                <div class="flex items-center gap-2">
                                    <button type="button"
                                        class="decrease px-3 py-1 bg-yellow-500 text-black font-bold rounded-l hover:bg-yellow-400">−</button>
                                    <input type="number" name="extras_entrantes[{{ $entrante->id }}]" min="0"
                                        value="0" data-precio="{{ $entrante->precio }}"
                                        class="cantidad-input w-14 sm:w-16 text-center text-black font-semibold rounded-none">
                                    <button type="button"
                                        class="increase px-3 py-1 bg-yellow-500 text-black font-bold rounded-r hover:bg-yellow-400">+</button>
                                </div>
