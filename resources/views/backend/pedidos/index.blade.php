    ...
    <div class="p-4 sm:p-6 bg-white rounded-xl shadow-md max-w-7xl mx-auto">
        <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2 mb-4">
            <a href="{{ route('admin.pedidos.index') }}"
                class="text-2xl sm:text-3xl font-extrabold text-blue-700 tracking-wide">
                Listado de Pedidos
            </a>

            <div class="flex flex-wrap gap-2">
                <a href="{{ route('admin.pedidos.hoy') }}"
                ...
                <a href="{{ route('admin.pedidos.semana') }}"
                    class="inline-block px-4 py-2 text-sm sm:text-base bg-indigo-600 text-white rounded-lg shadow hover:bg-indigo-700 focus:ring-4 focus:ring-indigo-300 transition font-semibold">
                    Ésta semana
                </a>
                <a href="{{ route('admin.pedidos.index') }}"
                ...
                <tbody class="divide-y divide-gray-100">
                    @foreach ($pedidos as $pedido)
                        @php
                            ...
                            $totalExtras = $pedido->pedidoProductos
                                ->flatMap(function ($unidad) {
                        ...
                        <tr class="hover:bg-gray-50 transition">
                            ...
                            <td class="px-4 py-2">{{ $pedido->reserva->cliente->nombre ?? ($pedido->reserva->nombre ?? '-') }}
                            </td>
